...
import { Table, TableBody, TableCell, TableHead, TableRow, Checkbox, Button, Avatar, Box } from '@mui/material';
import { useNavigate } from 'react-router-dom';
import axios from 'axios';

const CompanyTable = ({ companies, setCompanies, onDelete }) => {
            ...
            >
                ...
                <div style={{ display: 'flex', alignItems: 'center' }}>
                    <span style={{ marginRight: '16px' }}>
                        Showing {indexOfFirstCompany + 1}-{indexOfLastCompany} of {companies.length}
                    </span>
                        ...
                        <div>
                            ...
                            {Array.from({ length: Math.ceil(companies.length / companiesPerPage) }, (_, index) => (
                                <Button
                                    key={index}
                                    variant="contained"
                                    color={index + 1 === currentPage ? 'primary' : 'default'}
            ...
            <Table style={{ borderCollapse: 'collapse' }}>
                <TableHead>
                    <TableRow>
                        ...
                        <TableCell style={{ borderBottom: '1px solid #ddd' }}>SOCIAL PROFILES</TableCell>
                    ...
                    {currentCompanies.map((company) => (
                        <TableRow key={company._id} selected={selected.includes(company._id)} style={{ borderBottom: '1px solid #ddd' }}>
                            ...
                            <TableCell style={{ width: '40px' }}>
                                {company.logo ? (
                                    <img src={company.logo} alt="logo" style={{ width: '50px' }} />
                                ) : (
                                    ''
                                )}
                            </TableCell>
                            ...
                            <TableCell>
                                <Box display="flex" alignItems="center">
                                    {company.socialProfiles &&
                                        company.socialProfiles.map((profile, index) => (
                                            <Avatar
                                                key={index}
                                                src={profile.icon}
                                                alt={profile.name}
                            ...
                            <TableCell style={{ width: '40px' }}>
                                <Button variant="contained" color="primary" onClick={() => navigate(`/companies/${company._id}`)} style={{ padding: '8px 12px' }}>
                                    View
                                </Button>
                            </TableCell>
                        </TableRow>
